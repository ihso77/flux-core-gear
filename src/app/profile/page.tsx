...
import { User, Mail, Phone, MapPin, Gamepad2, Camera, Save, Loader2, LogOut, Shield, Bell, ShoppingBag, Heart, Settings, Edit3, Award } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    setIsLoading(true);
    const { error } = await updateProfile(editableData);
    setIsLoading(false);
          ...
          <div className="flex gap-3">
            <motion.button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl font-body text-sm font-medium transition-colors ${
                isEditing ? "bg-primary text-primary-foreground" : "bg-card border border-border text-foreground hover:border-primary/30"
              }`}
            >
            ...
            <div className="rounded-xl border border-border bg-card/50 p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {isEditing ? (
                    <input type="text" value={editableData.full_name} onChange={(e) => setEditableData({ ...editableData, full_name: e.target.value })} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary" />
                  ) : (
                    <p className="font-body text-sm text-foreground">{editableData.full_name || "—"}</p>
                  )}
                </div>
                <div>
                  <label className="block font-body text-sm text-muted-foreground mb-1">Username</label>
                  {isEditing ? (
                    <input type="text" value={editableData.username} onChange={(e) => setEditableData({ ...editableData, username: e.target.value })} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary" />
                  ) : (
                    <p className="font-body text-sm text-foreground">@{editableData.username || "—"}</p>
                  )}
                </div>
                <div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    <input type="tel" value={editableData.phone} onChange={(e) => setEditableData({ ...editableData, phone: e.target.value })} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary" />
                  ) : (
                    <p className="font-body text-sm text-foreground flex items-center gap-2">
            ...
            <div className="rounded-xl border border-border bg-card/50 p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  ...
                  {isEditing ? (
                    <input type="text" value={editableData.address_line1} onChange={(e) => setEditableData({ ...editableData, address_line1: e.target.value })} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary" />
                  ) : (
                    <p className="font-body text-sm text-foreground">{editableData.address_line1 || "—"}</p>
                  )}
                </div>
                <div>
                  <label className="block font-body text-sm text-muted-foreground mb-1">City</label>
                  {isEditing ? (
                    <input type="text" value={editableData.city} onChange={(e) => setEditableData({ ...editableData, city: e.target.value })} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary" />
                  ) : (
                    <p className="font-body text-sm text-foreground">{editableData.city || "—"}</p>
                  )}
                </div>
                <div>
                  <label className="block font-body text-sm text-muted-foreground mb-1">Country</label>
                  <p className="font-body text-sm text-foreground">{editableData.country}</p>
                </div>
              </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="space-y-6">
            ...
            <div className="rounded-xl border border-border bg-card/50 p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block font-body text-sm text-muted-foreground mb-1">Platform</label>
                  {isEditing ? (
                    <select value={editableData.preferred_platform} onChange={(e) => setEditableData({ ...editableData, preferred_platform: e.target.value })} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary">
                    ...
                    <p className="font-body text-sm text-foreground">{editableData.preferred_platform || "—"}</p>
                  )}
                </div>
                <div>
                  <label className="block font-body text-sm text-muted-foreground mb-1">Experience</label>
                  {isEditing ? (
                    <select value={editableData.gaming_experience} onChange={(e) => setEditableData({ ...editableData, gaming_experience: e.target.value })} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary">
                    ...
                    <p className="font-body text-sm text-foreground capitalize">{editableData.gaming_experience || "—"}</p>
                  )}
                </div>
              </div>
            </div>
